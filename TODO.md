...
| 中 | **26B config gap fix — E2E smoke ✅ shipped、merge ready** (2026-05-17 17:00 完了) | **背景**: 26B SO101 smoke で `segments=[{phase: idle}]` の degenerate 出力 (default boundary/smoother config が SO101 微弱グリッパー信号に反応せず)。4B vs 26B annotation richness fair compare の妨げ<br>**状態 (2026-05-17 18 時更新)**: 旧 `fix/26b-config-gap` (古い main 起点) は寝かせのまま放置。**新 branch `fix/batch-annotate-yaml-passthrough` を最新 `origin/main` (`cc6aa5e`) から作成し b17dd21 を cherry-pick 済 (local commit `0d65616`)** — push + PR 待ち<br>**Shipped (`b17dd21` → cherry-pick `0d65616`)**:<br>1. ✅ `scripts/run_26B_so101.sh` L49-50 — `--boundary-config` + `--smoother-config` 追加 (commit `7625a05`、本ブランチ前)<br>2. ✅ `scripts/batch_annotate.py` — `DATASETS` に optional `boundary_config` / `smoother_config` 追加、SO101 のみ 2 YAML 指定、gem4 は None で default fallback。`load_boundary_config_yaml` / `load_smoother_config_yaml` を `mimicanno.cli annotate` と同じパス経由で使用 (cli.py:175-176, 254-275)<br>3. ✅ `tests/test_batch_annotate_yaml_loading.py` — 5 unit tests passing (TDD red→green 確認済: fix なしで 4/5 fail、ありで 5/5 pass、再 cherry-pick 後も 5/5 PASS 確認 2026-05-17 18 時)。mypy: baseline 10 errors のまま、新規 0<br>**4B 検証 (新規・GPU 待たず即実行可、80GB GPU 不要)**: ⏳ **未実行**。Steps:<br>　a. `fix/batch-annotate-yaml-passthrough` チェックアウト<br>　b. `scripts/batch_annotate.py` の `ADAPTER_PATH` / `vlm_arg` を 4B (E4B-it) に差し替え (一時 patch でよい)、または 4B 用に複製した sibling script (`batch_annotate_4B_so101.py` 等) を用意<br>　c. SO101 ep0+ep1 のみ `BATCH_RUNS_ROOT=/tmp/g1_smoke_4b_v2/` で実走 (~2 min × 2 ep、A6000 1 枚で OK)<br>　d. `annotation.json` で `segments ≥ 2` + `boundaries.json` `candidates` 非空を verify<br>　e. 既存 `batch_so101_phase4_v5.sh` (別 4B 経路、proven) と segment 数で cross-check<br>　f. 結果を `2026-05-17-g1-26b-so101-smoke-results.md` に "4B passthrough verification" 節として追記<br>**Deferred (26B のみ)**: end-to-end 26B SO101 smoke は 80GB GPU 待ち。現環境全 4 GPU が RTX A6000 49GB で 26B (~52GB) OOM 確定。4B 検証 (上記) で経路の健全性は確認できるので、26B 実走は 80GB GPU 確保時で OK<br>**次に動かす条件**: 4B 検証 → PR push → ユーザー merge 承認 → 80GB GPU 確保時に 26B 本走<br>**詳細**: `docs/superpowers/notes/2026-05-17-g1-26b-so101-smoke-results.md` |
