...
- **Materializing the decoded label.** For each segment `s_t` whose decoded `q*_t != s_t.phase`, replace `s_t.phase` with `q*_t`, then materialize `verb` / `object` / `target` according to the rule:
  - If `q*_t` is an allowed label (present in the run's `labels.yaml`): **keep the segment's original `verb` / `object` / `target` values unchanged.** Rationale: the run's labelset (`mimicanno/labelset.py::Label`) carries only `id`, `verbs: list[str]` (allowed verb candidates, plural — not a canonical single verb), and `requires_object: bool`; it has **no** canonical `object` or `target` field. The original VLM-extracted argument structure is still the best information we have about the activity in the segment's frame range, and downstream consumers see `smoothing_ops` containing `"viterbi_relabel"` so they know the (verb, object, target) was originally tied to the *previous* phase choice. Phase 5's edit UI is responsible for surfacing relabeled segments for human review of the argument structure.
  - If `q*_t == "unknown"` (reserved label, not present in `labels.yaml`): set `verb = None`, `object = None`, `target = None` — parent spec §6.1 contract for reserved phases. This matches the shape Phase 2 emits when the VLM falls through to `phase="unknown"` after retry exhaustion.
