#!/usr/bin/env bash
# GEM4 replace_the_cookie — 26B QLoRA アダプタで推論
# Usage:
#   GPU=0 bash scripts/run_26B_gem4_replace_the_cookie.sh
#   GPU=0 START=0   END=107 bash scripts/run_26B_gem4_replace_the_cookie.sh &
#   GPU=1 START=108 END=215 bash scripts/run_26B_gem4_replace_the_cookie.sh
...
LOGS_DIR="${LOGS_DIR:-$REPO/logs/batch_gem4_replace_the_cookie_26B}"
...
echo "[gpu=$GPU] GEM4 replace_the_cookie 26B  episodes $START..$END"
cd "$REPO"
