"""--vlm-mask-overlay / --vlm-mask-alpha CLI flags plumb to VLMConfig."""
...
def test_default_enables_overlay_with_alpha_0_4(tmp_path: Path) -> None:
    captured, result = _run_with_captured_config(tmp_path, [])
    assert captured, f"pipeline never reached; output={result.output}"
    assert captured[0].mask_overlay.enabled is True
    assert captured[0].mask_overlay.alpha == pytest.approx(0.4)


def test_no_vlm_mask_overlay_disables(tmp_path: Path) -> None:
    captured, result = _run_with_captured_config(
        tmp_path, ["--no-vlm-mask-overlay"],
    )
    assert captured, f"pipeline never reached; output={result.output}"
    assert captured[0].mask_overlay.enabled is False


def test_vlm_mask_alpha_propagates(tmp_path: Path) -> None:
    captured, result = _run_with_captured_config(
        tmp_path, ["--vlm-mask-alpha", "0.6"],
    )
    assert captured, f"pipeline never reached; output={result.output}"
    assert captured[0].mask_overlay.alpha == pytest.approx(0.6)


def test_vlm_mask_alpha_out_of_range_rejected(tmp_path: Path) -> None:
    captured, result = _run_with_captured_config(
        tmp_path, ["--vlm-mask-alpha", "1.5"],
    )
    assert captured == []
    assert result.exit_code != 0
    assert "vlm-mask-alpha" in result.output or "alpha" in result.output
